...
* Simple code using the `{tidyverse}`, `{data.table}` and `{ggplot2}` + other packages.

* Package building tools including: `{devtools}`, `{usethis}`, `{testthat}`, `{vdiffr}`, and `{pgknet}`.

* R packages<sup>1</sup> by Hadley Wickham.

* Other tools: git, Travis CI, Appveyor, codevo.io etc.

* Copying `{dplyr}`<sup>2</sup> ......

.footnote[
[1] R packages: http://r-pkgs.had.co.nz, [1] `{dplyr}`: https://github.com/tidyverse/dplyr
]
...
## Paper: [bit.ly/getTBinR-paper](http://bit.ly/getTBinR-paper)

## Documentation: [bit.ly/getTBinR](http://bit.ly/getTBinR)

## Personal Website: [bit.ly/seabbs](http://bit.ly/seabbs)
